...
commods = {	('Ath','CHI'):{'q':0.3}, # nope, ath-> atl, atl-> chi			ANSWERS FOR QUESTION 2
        ('Ath','DAL'):{'q':0.4}, # nope, just ath->atl, atl->dal
        ('Mac','ATL'):{'q':0.7}, # use several paths. Use mac->atl and mac->dal, dal->atl
        ('Mac','DAL'):{'q':0.5},  # nope mac-> dal
        ('Mac','CHI'):{'q':0.6}, #  mac-> dal, dal->chi 0.2 and mac->atl, atl-> chi 0.4
        ('ATL','CHI'):{'q':1.5}, # nope
        ('ATL','DAL'):{'q':2.2}, # nope atl->dal
        ('DAL','CHI'):{'q':1.7}, # Nope dal->chi
        ('DAL','ATL'):{'q':1.1}, # nope dal->atl
        ('DAL','Ath'):{'q':0.6} # nope  dal->atl, atl->ath
}
...
for a, a_dict in arcs.iteritems():
    ...
    for dest in destinations:
        l = dest
        var = pulp.LpVariable("ArcFlow(%s,%s)_(%s)" % (str(i),str(j),str(l)), lowBound = 0,cat=pulp.LpBinary)
        a_dict['onePath'][l] = var
...
for a,a_dict in arcs.iteritems():
    ...
    # constraint only one path is taken
    for d in destinations:
        prob += pulp.lpSum(arcs[a]['onePath'][d] for a in nodes[i]['outArcs']) == 1

for a, a_dict in arcs.iteritems():
	for commd, val in commods.iteritems():
		d = commd[1]
		prob += a_dict['dvFlows'][commd] <= val['q'] * a_dict['onePath'][d]
